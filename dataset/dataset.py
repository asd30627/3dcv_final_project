...
class NuScenesDataset(Dataset):
    ...
    def __init__(
        ...
        vis_scene_index=-1,
        phase='train',
        # NuScenesDataset(..., return_keys=[ ... ]) 這裡加上下面四個
        return_keys=[
            'phase',
            'sample_idx',
            'img_filename',
    ...
    def __getitem__(self, index):
        # index = dataloader 全域 index (0..len-1)
        scene_token, frame_idx = self.keyframes[index]
        info = deepcopy(self.scene_infos[scene_token][frame_idx])

        input_dict = self.get_data_info(info)

        # ✅ 給 teacher dump 用：一定要用全域 index
        input_dict["dump_idx"] = int(index)

        # （可選）你想 debug 才留
        input_dict["frame_idx"] = int(frame_idx)
        input_dict["scene_token"] = scene_token

        if self.data_aug_conf is not None:
            input_dict["aug_configs"] = self._sample_augmentation()

        input_dict["phase"] = self.phase

        for t in self.pipeline:
            input_dict = t(input_dict)

        return {k: input_dict.get(k, None) for k in self.return_keys}
